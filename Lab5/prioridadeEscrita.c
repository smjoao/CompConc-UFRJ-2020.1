...
int queresc = 0;  // Num de threads escritoras que estao esperando para escrever
...
void EntraLeitura(int id) {
    ...
    // mas verifica também se alguma thread está esperando para escrever
    while(esc > 0 || queresc > 0) {
...
void SaiEscrita(int id) {
    ...
    if(queresc == 0) pthread_cond_broadcast(&condLeit);
    pthread_mutex_unlock(&mutex);
}
